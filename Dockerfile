...
RUN go get -v -d github.com/georzaza/go-ethereum-v0.7.10_official/...
WORKDIR $GOPATH/src/github.com/georzaza/go-ethereum-v0.7.10_official
RUN git checkout poc8
RUN ETH_DEPS=$(go list -f '{{.Imports}} {{.TestImports}} {{.XTestImports}}' github.com/georzaza/go-ethereum-v0.7.10_official/... | sed -e 's/\[//g' | sed -e 's/\]//g' | sed -e 's/C //g'); if [ "$ETH_DEPS" ]; then go get $ETH_DEPS; fi
